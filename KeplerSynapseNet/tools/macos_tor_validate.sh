...
RUN_DIR="$OUTPUT_ROOT/macos_tor_validate_$RUN_ID"
...
usage() {
    cat <<'EOF'
Usage: tools/macos_tor_validate.sh [options]

Validate SynapseNet Tor runtime behavior on macOS:
...
require_macos() {
    ...
    [[ "$os" == "Darwin" ]] || die "this validator is macOS-only (detected: ${os:-unknown})"
}
...
external_socks_owner_hint() {
    local details
    details="$(listener_owner_details "$EXTERNAL_SOCKS_PORT" || true)"
    [[ -n "$details" ]] || return 0
    if [[ "$details" == *"Tor Browser.app"* ]]; then
        if [[ "$details" == *"DisableNetwork 1"* ]]; then
            printf 'owner=%s (Tor Browser runtime with DisableNetwork=1; use external Tor process on 9150 or switch Tor Browser to external SOCKS client mode)' "$details"
            return 0
        fi
        printf 'owner=%s (Tor Browser runtime owns this SOCKS port)' "$details"
        return 0
    fi
    printf 'owner=%s' "$details"
}
...
validate_external_mode() {
    ...
    local owner_hint
    owner_hint="$(external_socks_owner_hint || true)"
    if [[ -n "$owner_hint" ]]; then
        log "external SOCKS listener hint: $owner_hint"
    fi
    ...
    if [[ "$REQUIRE_EXTERNAL_WEB_READY" == "1" && "$web_ready" != "true" ]]; then
        if [[ -n "$owner_hint" ]]; then
            record_failure "external mode: expected torReadyForWeb=true got '$web_ready' ($owner_hint)"
        else
    ...
    if ! curl --silent --show-error \
        ...
        if [[ -n "$owner_hint" ]]; then
            record_failure "external mode: SOCKS probe failed via ${EXTERNAL_SOCKS_HOST}:${EXTERNAL_SOCKS_PORT} ($owner_hint)"
        else
