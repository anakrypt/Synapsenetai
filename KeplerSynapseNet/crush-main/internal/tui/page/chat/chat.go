...
import (
	...
	unipatch "github.com/charmbracelet/crush/internal/patch"
	...
	patchdlg "github.com/charmbracelet/crush/internal/tui/components/dialogs/patch"
...
type chatPage struct {
	...
	patchAwaiting             bool
	patchAwaitingSessionID    string
	patchLastAssistantMessage string
}
...
func (p *chatPage) Update(msg tea.Msg) (util.Model, tea.Cmd) {
	...
	case editor.OpenEditorMsg:
		...
		return p, cmd
	case chat.SendMsg:
		if cmd := p.handleCommand(msg.Text, msg.Attachments); cmd != nil {
			return p, cmd
		}
	...
	case commands.ToggleWeb4Msg:
		return p, p.toggleWeb4()
	case commands.ToggleWeb4OnionMsg:
		return p, p.toggleWeb4Onion()
	case commands.ToggleWeb4TorMsg:
		return p, p.toggleWeb4Tor()
	...
	case pubsub.Event[message.Message],
		anim.StepMsg,
		spinner.TickMsg:
		if ev, ok := msg.(pubsub.Event[message.Message]); ok && p.patchAwaiting && ev.Payload.SessionID == p.patchAwaitingSessionID {
			if ev.Type == pubsub.CreatedEvent && ev.Payload.Role == message.Assistant {
				p.patchLastAssistantMessage = ev.Payload.ID
			}
			if ev.Type == pubsub.UpdatedEvent && ev.Payload.ID != "" && ev.Payload.ID == p.patchLastAssistantMessage && ev.Payload.Role == message.Assistant && ev.Payload.IsFinished() {
				p.patchAwaiting = false
				p.patchAwaitingSessionID = ""
				p.patchLastAssistantMessage = ""
				text := extractMessageText(ev.Payload)
				patches, err := unipatch.ParseUnifiedDiff(text)
				if err != nil {
					cmds = append(cmds, util.ReportError(err))
				} else {
					cmds = append(cmds, util.CmdHandler(dialogs.OpenDialogMsg{
						Model: patchdlg.New(p.app, ev.Payload.SessionID, patches),
					}))
				}
			}
		}
	...
	case tea.KeyPressMsg:
		...
		case key.Matches(msg, p.keyMap.ToggleWeb4):
			if p.isSynapseNetProvider() {
				return p, p.toggleWeb4()
			}
		case key.Matches(msg, p.keyMap.ToggleWeb4Onion):
			if p.isSynapseNetProvider() {
				return p, p.toggleWeb4Onion()
			}
		case key.Matches(msg, p.keyMap.ToggleWeb4Tor):
			if p.isSynapseNetProvider() {
				return p, p.toggleWeb4Tor()
			}
		}
...
func (p *chatPage) handleCommand(text string, attachments []message.Attachment) tea.Cmd {
	...
	switch {
	case strings.HasPrefix(trimmed, "/tangent"):
		title := strings.TrimSpace(strings.TrimPrefix(trimmed, "/tangent"))
		if title == "" {
			title = "Tangent"
		}
		if p.session.ID == "" {
			sess, err := p.app.Sessions.Create(context.Background(), "")
			if err != nil {
				return util.ReportError(err)
			}
			p.session = sess
		}
		thread, err := p.app.Sessions.CreateThreadSession(context.Background(), p.session.ID, title)
		if err != nil {
			return util.ReportError(err)
		}
		return tea.Batch(
			util.CmdHandler(chat.SessionSelectedMsg(thread)),
			util.ReportInfo("Switched to tangent thread"),
		)
	case strings.HasPrefix(trimmed, "/up"):
		if p.session.ID == "" {
			return nil
		}
		if p.session.ParentSessionID == "" {
			return util.ReportWarn("No parent thread")
		}
		parent, err := p.app.Sessions.Get(context.Background(), p.session.ParentSessionID)
		if err != nil {
			return util.ReportError(err)
		}
		return tea.Batch(
			util.CmdHandler(chat.SessionSelectedMsg(parent)),
			util.ReportInfo("Switched to parent thread"),
		)
	case strings.HasPrefix(trimmed, "/patch"):
		prompt := strings.TrimSpace(strings.TrimPrefix(trimmed, "/patch"))
		if prompt == "" {
			return util.ReportWarn("Usage: /patch <instruction>")
		}

		p.patchAwaiting = true
		p.patchLastAssistantMessage = ""
		...
		if p.session.ID == "" {
			...
			p.patchAwaitingSessionID = sess.ID
			return tea.Sequence(
				util.CmdHandler(chat.SessionSelectedMsg(sess)),
				util.CmdHandler(chat.SendMsg{
					Text:        buildPatchPrompt(prompt),
					Attachments: attachments,
				}),
				util.ReportInfo("Patch request sent"),
			)
		}

		p.patchAwaitingSessionID = p.session.ID
		return tea.Batch(
			p.sendMessage(buildPatchPrompt(prompt), attachments),
			util.ReportInfo("Patch request sent"),
		)
	default:
		return nil
...
func extractMessageText(m message.Message) string {
...
func buildPatchPrompt(instruction string) string {
	return "Return only a unified diff starting with `diff --git`.\n" +
...
func (p *chatPage) toggleWeb4() tea.Cmd {
	return func() tea.Msg {
		cfg := config.Get()
		if cfg.Options == nil {
		...
		if cfg.Options.TUI.Web4 == nil {
			cfg.Options.TUI.Web4 = &config.Web4Options{}
		}

		// Toggle Web4 injection
		enabled := !cfg.Options.TUI.Web4.InjectEnabled
		if err := cfg.SetWeb4Inject(enabled); err != nil {
			return util.InfoMsg{
				Type: util.InfoTypeError,
				Msg:  "Failed to update Web4 injection: " + err.Error(),
			}
		}
		...
		return util.InfoMsg{
			Type: util.InfoTypeInfo,
			Msg:  "Web4 injection " + status,
		}
	}
}

func (p *chatPage) toggleWeb4Onion() tea.Cmd {
	return func() tea.Msg {
		cfg := config.Get()
		if cfg.Options == nil || cfg.Options.TUI == nil || cfg.Options.TUI.Web4 == nil {
			return util.InfoMsg{
				Type: util.InfoTypeError,
				Msg:  "Web4 injection must be enabled first",
			}
		}

		if !cfg.Options.TUI.Web4.InjectEnabled {
			return util.InfoMsg{
				Type: util.InfoTypeError,
				Msg:  "Web4 injection must be enabled first",
			}
		...
		enabled := !cfg.Options.TUI.Web4.OnionEnabled
		if err := cfg.SetWeb4Onion(enabled); err != nil {
			return util.InfoMsg{
				Type: util.InfoTypeError,
				Msg:  "Failed to update Onion sources: " + err.Error(),
			}
		}
		...
		return util.InfoMsg{
			Type: util.InfoTypeInfo,
			Msg:  "Onion sources " + status,
		}
	}
}

func (p *chatPage) toggleWeb4Tor() tea.Cmd {
	return func() tea.Msg {
		cfg := config.Get()
		if cfg.Options == nil || cfg.Options.TUI == nil || cfg.Options.TUI.Web4 == nil {
			return util.InfoMsg{
		...
		enabled := !cfg.Options.TUI.Web4.TorClearnet
		if err := cfg.SetWeb4Tor(enabled); err != nil {
			return util.InfoMsg{
				Type: util.InfoTypeError,
				Msg:  "Failed to update Tor routing: " + err.Error(),
			}
		}
		...
		return util.InfoMsg{
			Type: util.InfoTypeInfo,
			Msg:  "Tor for clearnet " + status,
		}
	}
}

func (p *chatPage) isSynapseNetProvider() bool {
	cfg := config.Get()
	agentCfg := cfg.Agents[config.AgentCoder]
	modelProvider := cfg.GetProviderForModel(agentCfg.Model)
	return modelProvider != nil && modelProvider.Type == synapsenet.Name
}
