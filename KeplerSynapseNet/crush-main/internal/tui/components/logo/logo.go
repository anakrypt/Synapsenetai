// Package logo renders a SynapseNet wordmark in a stylized way.
package logo
...
// Render renders the SynapseNet logo. Set the argument to true to render the narrow
...
func Render(version string, compact bool, o Opts) string {
	...
	title := styles.ApplyBoldForegroundGrad(titlePlain, o.TitleColorA, o.TitleColorB)
	titleWidth := lipgloss.Width(titlePlain)
	...
	// Narrow version.
	if compact {
		field := fg(o.FieldColor, strings.Repeat(diag, wordmarkWidth))
	...
	// Right field.
	rightWidth := max(15, o.Width-wordmarkWidth-leftWidth-2) // 2 for the gap.
	const stepDownAt = 0
...
// SmallRender renders a smaller version of the logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(width int) string {
	t := styles.CurrentTheme()
	title := t.S().Base.Foreground(t.Secondary).Render("Kepler")
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad("SynapseNet", t.Secondary, t.Primary))
