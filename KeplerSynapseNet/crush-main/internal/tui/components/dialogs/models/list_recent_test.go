...
func TestModelList_RecentlyUsedSectionAndPrunesInvalid(t *testing.T) {
	...
	confPath := filepath.Join(cfgDir, "synapseide", "synapseide.json")
	...
	initial := map[string]any{
		...
		"providers": map[string]any{
			"synapsenet": map[string]any{
				"base_url": "http://127.0.0.1:8332",
				"models": []any{
					map[string]any{"id": "m1", "name": "Model One", "default_max_tokens": 100},
					map[string]any{"id": "m2", "name": "Model Two", "default_max_tokens": 100},
				},
			},
		},
		...
		"recent_models": map[string]any{
			"large": []any{
				map[string]any{"model": "m2", "provider": "synapsenet"},      // valid
	...
	dataConfDir := filepath.Join(dataDir, "synapseide")
	...
	origConfPath := filepath.Join(cfgDir, "synapseide", "synapseide.json")
	...
	dataConf := filepath.Join(dataDir, "synapseide", "synapseide.json")
	...
	for _, v := range largeAny {
		...
		if m["provider"] == "synapsenet" && m["model"] == "m2" {
			found = true
		}
	}
	require.True(t, found, "persisted recents should include synapsenet:m2")
}
...
func TestModelList_PrunesInvalidModelWithinValidProvider(t *testing.T) {
	...
	initial := map[string]any{
		...
		"recent_models": map[string]any{
			"large": []any{
				map[string]any{"model": "m1", "provider": "synapsenet"},      // valid
				map[string]any{"model": "missing", "provider": "synapsenet"}, // invalid model
			},
		},
	}
	bts, err := json.Marshal(initial)
	...
	foundValid := false
	for _, it := range recentItems {
		if it.ID() == "recent::synapsenet:m1" {
			foundValid = true
		}
		require.NotEqual(t, "recent::synapsenet:missing", it.ID(), "invalid model should be pruned")
	}
	require.True(t, foundValid, "valid recent synapsenet:m1 should be present")
	...
	// Verify only synapsenet:m1 remains
	m := largeAny[0].(map[string]any)
	require.Equal(t, "synapsenet", m["provider"])
