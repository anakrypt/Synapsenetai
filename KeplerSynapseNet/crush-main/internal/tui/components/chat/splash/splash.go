...
func (s *splashCmp) logoBlock() string {
	...
	return logoStyle.Render(
		logo.DotMatrixRender(version.Version, false, logo.Opts{
...
func (s *splashCmp) tipsBlock() string {
	t := styles.CurrentTheme()
	maxWidth := s.getMaxInfoWidth()
	section := t.S().Subtle.Render("Tips")
	lineStyle := t.S().Muted.Width(maxWidth)
	lines := []string{
		section,
		"",
		lineStyle.Render("/patch <instruction>  diff → preview/apply/submit"),
		lineStyle.Render("/tangent <title>     new thread, /up to parent"),
		lineStyle.Render("ctrl+j               newline"),
		lineStyle.Render("ctrl+s               sessions"),
	}
	return t.S().Base.Width(maxWidth).Render(lipgloss.JoinVertical(lipgloss.Left, lines...))
}
...
func (s *splashCmp) currentModelBlock() string {
	...
	if cfg.Options != nil && cfg.Options.TUI != nil && cfg.Options.TUI.Remote != nil {
		...
		if remote.Enabled && strings.TrimSpace(remote.SessionID) != "" {
			parts = append(parts, t.S().Muted.Render("Remote: "+shortenID(remote.SessionID)))
		}
	}
