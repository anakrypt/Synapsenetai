...
func (m *sidebarCmp) View() string {
	...
	if m.compactMode && m.width > m.height {
		...
		if m.isSynapseNetProvider() {
			parts = append(parts, "", m.web4Block())
		}
	}
...
func (m *sidebarCmp) isSynapseNetProvider() bool {
	cfg := config.Get()
...
func (m *sidebarCmp) web4Block() string {
	cfg := config.Get()
	t := styles.CurrentTheme()
	
	var web4Status string
	var onionStatus string
	...
	sectionName := core.Section("Web4", m.getMaxWidth())
	web4Line := fmt.Sprintf("  Web4: %s", web4Status)
	if web4Status == "ON" {
		web4Line += fmt.Sprintf(" | Onion: %s | Tor: %s", onionStatus, torStatus)
	}
	
	web4Content := t.S().Subtle.Render(web4Line)
