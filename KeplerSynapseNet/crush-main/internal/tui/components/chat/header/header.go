...
func (h *header) View() string {
	...
	b.WriteString(t.S().Base.Foreground(t.Secondary).Render("Kepler"))
	b.WriteString(gap)
	b.WriteString(styles.ApplyBoldForegroundGrad("SYNAPSENET", t.Secondary, t.Primary))
	b.WriteString(gap)
...
func (h *header) details(availWidth int) string {
	...
	// Web4 toggles status (always visible when space allows).
	webEnabled := false
	onionEnabled := false
	torEnabled := false
	cfg := config.Get()
	if cfg.Options != nil && cfg.Options.TUI != nil && cfg.Options.TUI.Web4 != nil {
		webEnabled = cfg.Options.TUI.Web4.InjectEnabled
		onionEnabled = cfg.Options.TUI.Web4.OnionEnabled
		torEnabled = cfg.Options.TUI.Web4.TorClearnet
	}

	tag := func(label string, on bool) string {
		if on {
			return s.Base.Foreground(t.Secondary).Bold(true).Render(label)
		}
	...
	parts = append(parts,
		tag("WEB", webEnabled),
		tag("ONION", webEnabled && onionEnabled),
		tag("TOR", webEnabled && torEnabled),
	)

	questIssue := 0
	if cfg.Options != nil && cfg.Options.TUI != nil && cfg.Options.TUI.Quests != nil {
		questIssue = cfg.Options.TUI.Quests.ActiveIssue
	}
	if questIssue > 0 {
		parts = append(parts, s.Base.Foreground(t.Secondary).Bold(true).Render(fmt.Sprintf("Q#%d", questIssue)))
	}
