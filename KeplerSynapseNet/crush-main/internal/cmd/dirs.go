...
var dirsCmd = &cobra.Command{
	Use:   "dirs",
	Short: "Print directories used by SynapseIDE",
	Long: `Print the directories where SynapseIDE stores its configuration and data files.
...
synapseide dirs config

# Print only the data directory
synapseide dirs data
  `,
...
var configDirCmd = &cobra.Command{
	Use:   "config",
	Short: "Print the configuration directory used by SynapseIDE",
	Run: func(cmd *cobra.Command, args []string) {
		cmd.Println(filepath.Dir(config.GlobalConfig()))
	},
}
...
var dataDirCmd = &cobra.Command{
	Use:   "data",
	Short: "Print the data directory used by SynapseIDE",
	Run: func(cmd *cobra.Command, args []string) {
		cmd.Println(filepath.Dir(config.GlobalConfigData()))
	},
}
