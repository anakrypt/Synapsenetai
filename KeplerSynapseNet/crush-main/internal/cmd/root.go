...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom synapseide data directory")
...
var rootCmd = &cobra.Command{
	Use:   "synapseide",
	Short: "SynapseNet terminal IDE (local-only)",
	Long:  "SynapseNet terminal IDE powered by local GGUF inference via synapsed RPC (no centralized providers).",
	Example: `
...
synapseide -d -c /path/to/project

# Run with custom data directory
synapseide -D /path/to/custom/.synapseide

# Print version
synapseide -v

# Run a single non-interactive prompt
synapseide run "Explain the use of context in Go"
  ...
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := program.Run(); err != nil {
			...
			return errors.New("SynapseIDE crashed. If you'd like to report it, please copy the stacktrace above.") //nolint:staticcheck
		}
		return nil
	},
...
func setupApp(cmd *cobra.Command) (*app.App, error) {
	...
	if err := ensureDataDir(cfg.Options.DataDirectory); err != nil {
		return nil, err
	}
...
func shouldEnableMetrics() bool {
	if v, _ := strconv.ParseBool(os.Getenv("SYNAPSEIDE_DISABLE_METRICS")); v {
		return false
	}
