...
func TestCoderAgent(t *testing.T) {
	...
	if os.Getenv("SYNAPSEIDE_REMOTE_AGENT_TESTS") == "" {
		t.Skip("skipping remote-provider VCR tests in local-only SynapseIDE build")
	}
	...
	for _, pair := range modelPairs {
		t.Run(pair.name, func(t *testing.T) {
			...
			t.Run("update a file", func(t *testing.T) {
				...
				res, err := agent.Run(t.Context(), SessionAgentCall{
					Prompt:          "update the main.go file by changing the print to say hello from synapseide",
					SessionID:       session.ID,
					MaxOutputTokens: 10000,
				})
				require.NoError(t, err)
				...
				require.NoError(t, err)
				require.Contains(t, strings.ToLower(string(content)), "hello from synapseide")
			})
