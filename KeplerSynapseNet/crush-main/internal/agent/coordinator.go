...
func (c *coordinator) buildSynapseNetProvider(baseURL string, headers map[string]string) (fantasy.Provider, error) {
	opts := []synapsenet.Option{
		synapsenet.WithBaseURL(baseURL),
	}
	if c.cfg.Options.Debug {
		httpClient := log.NewHTTPClient()
		opts = append(opts, synapsenet.WithHTTPClient(httpClient))
	}
	_ = headers
	return synapsenet.New(opts...)
}
...
func (c *coordinator) buildProvider(providerCfg config.ProviderConfig, model config.SelectedModel, isSubAgent bool) (fantasy.Provider, error) {
	...
	case synapsenet.Name:
		return c.buildSynapseNetProvider(baseURL, headers)
	default:
