...
const (
	appName              = "synapseide"
	defaultDataDirectory = ".synapseide"
...
type TUIOptions struct {
	...
	Web4        *Web4Options   `json:"web4,omitempty" jsonschema:"description=Web4 search injection options"`
	Remote      *RemoteOptions `json:"remote,omitempty" jsonschema:"description=Remote inference (provider rental) options"`
	Quests      *QuestOptions  `json:"quests,omitempty" jsonschema:"description=GitHub Quests options"`
}

// Web4Options defines options for Web4 web search injection.
type Web4Options struct {
	InjectEnabled bool   `json:"inject_enabled,omitempty" jsonschema:"description=Enable web context injection,default=false"`
	OnionEnabled  bool   `json:"onion_enabled,omitempty" jsonschema:"description=Include onion sources in searches,default=false"`
	TorClearnet   bool   `json:"tor_clearnet,omitempty" jsonschema:"description=Route clearnet requests through Tor,default=false"`
	Query         string `json:"query,omitempty" jsonschema:"description=Optional explicit search query"`
}

// RemoteOptions defines opt-in remote model usage. This passes through to synapsed RPC
// (which remains local-first by default).
type RemoteOptions struct {
	Enabled   bool   `json:"enabled,omitempty" jsonschema:"description=Enable remote inference routing (opt-in),default=false"`
	SessionID string `json:"session_id,omitempty" jsonschema:"description=Remote rental session id obtained via model.remote.rent"`
}

type QuestOptions struct {
	Enabled          bool   `json:"enabled,omitempty" jsonschema:"description=Enable GitHub Quests UI,default=false"`
	Repo             string `json:"repo,omitempty" jsonschema:"description=GitHub repo in owner/name form"`
	Token            string `json:"token,omitempty" jsonschema:"description=GitHub token (optional)"`
	ActiveIssue      int    `json:"active_issue,omitempty" jsonschema:"description=Selected quest issue number"`
	ActiveTitle      string `json:"active_title,omitempty" jsonschema:"description=Selected quest title"`
	ActiveURL        string `json:"active_url,omitempty" jsonschema:"description=Selected quest URL"`
	MinSubmitPowBits int    `json:"min_submit_pow_bits,omitempty" jsonschema:"description=Minimum leading-zero bits required for PoE submitId attached to PR,default=16"`
	LastForkAt       int64  `json:"last_fork_at,omitempty" jsonschema:"description=Unix timestamp of last fork request"`
	LastPRAt         int64  `json:"last_pr_at,omitempty" jsonschema:"description=Unix timestamp of last PR creation request"`
}
...
type Attribution struct {
	...
	GeneratedWith bool         `json:"generated_with,omitempty" jsonschema:"description=Add Generated with SynapseIDE line to commit messages and issues and PRs,default=true"`
}
...
type Options struct {
	...
	SkillsPaths               []string     `json:"skills_paths,omitempty" jsonschema:"description=Paths to directories containing Agent Skills (folders with SKILL.md files),example=~/.config/synapseide/skills,example=./skills"`
	TUI                       *TUIOptions  `json:"tui,omitempty" jsonschema:"description=Terminal user interface options"`
	Debug                     bool         `json:"debug,omitempty" jsonschema:"description=Enable debug logging,default=false"`
	DebugLSP                  bool         `json:"debug_lsp,omitempty" jsonschema:"description=Enable debug logging for LSP servers,default=false"`
	DisableAutoSummarize      bool         `json:"disable_auto_summarize,omitempty" jsonschema:"description=Disable automatic conversation summarization,default=false"`
	DataDirectory             string       `json:"data_directory,omitempty" jsonschema:"description=Directory for storing application data (relative to working directory),default=.synapseide,example=.synapseide"` // Relative to the cwd
...
func (c *Config) SetWeb4Inject(enabled bool) error {
	if c.Options == nil {
		c.Options = &Options{}
	}
	if c.Options.TUI == nil {
	...
	if c.Options.TUI.Web4 == nil {
		c.Options.TUI.Web4 = &Web4Options{}
	}
	c.Options.TUI.Web4.InjectEnabled = enabled
	return c.SetConfigField("options.tui.web4.inject_enabled", enabled)
}

func (c *Config) SetWeb4Onion(enabled bool) error {
	if c.Options == nil {
		c.Options = &Options{}
	}
	if c.Options.TUI == nil {
	...
	c.Options.TUI.Web4.OnionEnabled = enabled
	return c.SetConfigField("options.tui.web4.onion_enabled", enabled)
}

func (c *Config) SetWeb4Tor(enabled bool) error {
	if c.Options == nil {
		c.Options = &Options{}
	}
	if c.Options.TUI == nil {
	...
	c.Options.TUI.Web4.TorClearnet = enabled
	return c.SetConfigField("options.tui.web4.tor_clearnet", enabled)
}
