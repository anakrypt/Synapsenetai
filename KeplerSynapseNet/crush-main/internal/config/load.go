...
func PushPopSynapseIDEEnv() func() {
	type candidate struct {
		value       string
		priorityIdx int
	}
	prefixes := []string{"SYNAPSEIDE_", "CRUSH_"}
	...
	for _, pairStr := range os.Environ() {
		...
		for i, prefix := range prefixes {
			...
			if cur, ok := candidates[base]; ok && cur.priorityIdx <= i {
				continue
			}
			candidates[base] = candidate{value: value, priorityIdx: i}
		}
	}
	...
	backups := make(map[string]backup, len(candidates))
	for base := range candidates {
		val, ok := os.LookupEnv(base)
		backups[base] = backup{value: val, ok: ok}
	}

	for base, cand := range candidates {
		_ = os.Setenv(base, cand.value)
	}

	return func() {
		for base, b := range backups {
			if b.ok {
				_ = os.Setenv(base, b.value)
				continue
			}
			_ = os.Unsetenv(base)
		}
...
func (c *Config) configureProviders(env env.Env, resolver VariableResolver, knownProviders []catwalk.Provider) error {
	...
	for id, providerConfig := range c.Providers.Seq2() {
		...
		if providerConfig.Type == "" {
			providerConfig.Type = "synapsenet"
		}
		if providerConfig.Type != "synapsenet" {
			slog.Warn("Skipping custom provider: only synapsenet providers are supported in this build", "provider", id)
			c.Providers.Del(id)
			continue
		}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("SYNAPSEIDE_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	} else if str, ok := os.LookupEnv("CRUSH_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("SYNAPSEIDE_DISABLE_DEFAULT_PROVIDERS"); ok {
		c.Options.DisableDefaultProviders, _ = strconv.ParseBool(str)
	} else if str, ok := os.LookupEnv("CRUSH_DISABLE_DEFAULT_PROVIDERS"); ok {
		c.Options.DisableDefaultProviders, _ = strconv.ParseBool(str)
	}
	...
	defaultModels := []catwalk.Model{
		{ID: "deepseek-coder-6.7b-instruct.Q4_K_M.gguf", Name: "DeepSeek Coder 6.7B Instruct (Q4_K_M)", ContextWindow: 8192, DefaultMaxTokens: 512},
		{ID: "active", Name: "Active Local Model", ContextWindow: 8192, DefaultMaxTokens: 512},
	}
	defaultBaseURL := cmp.Or(os.Getenv("SYNAPSENET_RPC_URL"), "http://127.0.0.1:8332")

	if pc, ok := c.Providers.Get("synapsenet"); ok {
		if pc.ID == "" {
			pc.ID = "synapsenet"
		}
		if pc.Name == "" {
			pc.Name = "SynapseNet (Local)"
		}
		if pc.BaseURL == "" {
			pc.BaseURL = defaultBaseURL
		}
		pc.Type = "synapsenet"
		pc.APIKey = ""
		if len(pc.Models) == 0 {
			pc.Models = defaultModels
		}
		c.Providers.Set("synapsenet", pc)
	} else {
		c.Providers.Set("synapsenet", ProviderConfig{
			ID:      "synapsenet",
			Name:    "SynapseNet (Local)",
...
func GlobalConfig() string {
	if global := os.Getenv("SYNAPSEIDE_GLOBAL_CONFIG"); global != "" {
		return filepath.Join(global, fmt.Sprintf("%s.json", appName))
	}
	if global := os.Getenv("CRUSH_GLOBAL_CONFIG"); global != "" {
		return filepath.Join(global, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalConfigData() string {
	if data := os.Getenv("SYNAPSEIDE_GLOBAL_DATA"); data != "" {
		return filepath.Join(data, fmt.Sprintf("%s.json", appName))
	}
	if data := os.Getenv("CRUSH_GLOBAL_DATA"); data != "" {
		return filepath.Join(data, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalSkillsDirs() []string {
	if dir := os.Getenv("SYNAPSEIDE_SKILLS_DIR"); dir != "" {
		return []string{dir}
	}
