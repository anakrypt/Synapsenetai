	...
	providerOnce.Do(func() {
		...
		wg.Go(func() {
			...
			if err != nil {
				...
				errs = append(errs, fmt.Errorf("SynapseIDE was unable to fetch an updated list of providers from %s.\n\nCause: %w", catwalkURL, providerErr)) //nolint:staticcheck
				return
			}
		...
		wg.Go(func() {
			...
			if err != nil {
				errs = append(errs, fmt.Errorf("SynapseIDE was unable to fetch updated information from Hyper: %w", err)) //nolint:staticcheck
				return
			}
