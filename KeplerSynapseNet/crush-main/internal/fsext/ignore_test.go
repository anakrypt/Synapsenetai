...
func TestSynapseIDEIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .synapseideignore file that ignores .log files
	require.NoError(t, os.WriteFile(".synapseideignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .synapseideignore file
	ignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".synapseideignore"), []byte(ignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .synapseideignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .synapseideignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .synapseideignore in subdir that ignores normal_nested
	subIgnore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".synapseideignore"), []byte(subIgnore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .synapseideignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .synapseideignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .synapseideignore")
